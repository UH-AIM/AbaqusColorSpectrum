...
def ABA_color():
    ...
    colorStrs = ['viridis', 'plasma','inferno','magma', 'cividis']
    # see more color gradients here https://matplotlib.org/stable/tutorials/colors/colormaps.html
    colorLists = [[mpl.colors.to_hex(x[0:3]).encode('ascii', 'ignore') for x in plt.get_cmap(colorStr, 100).colors] for colorStr in colorStrs]
    ...
    [session.Spectrum(name='colorGradient-'+colorStr, colors = colorList) for colorStr, colorList in zip(colorStrs,colorLists)]
